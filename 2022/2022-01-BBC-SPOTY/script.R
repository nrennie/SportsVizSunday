...
p <- ggplot() +
  ...
  labs(title = "BBC Sports Personality of the Year",
  ...
  theme_void() +
  coord_fixed() +
  theme(plot.title = element_text(colour = "white", size=24, family="libre", face="bold", hjust=0.5, vjust=0, margin = margin(5,2,15,2)),
        plot.subtitle = element_markdown(family = "sans", hjust = 0.5, size = 12, color = "white", lineheight = 1.4, margin = margin(15,2,15,2)),
...
q <- ggdraw() +
  ...
  draw_label(label=str_wrap("In 1962, English swimmer Anita Lonsbrough becomes the first female recipient of the BBC SPOTY.", 30),
             x=0.34, y=0.19, hjust=0.5, fontfamily="justanotherhand", size=16, colour = "white") +
  ...
  draw_label(label=str_wrap("In 2021, tennis player Emma Raducanu won after 14 consecutive years of male winners.", 30),
             x=0.92, y=0.63, hjust=0.5, fontfamily="justanotherhand", size=16, colour = "white") +
